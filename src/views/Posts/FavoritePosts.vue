...
const favoritePosts = ref([
  {
    ...
    excerpt: 'Exploring how AI tools are revolutionizing the way developers build and optimize websites and applications.',
    category: 'articles',
    readTime: 7,
    date: 'Apr 15, 2025',
    author: {
...
const filteredPosts = computed(() => {
  if (activeCategory.value === 'all') {
    ...
    return favoritePosts.value.filter(post => post.category === activeCategory.value);
  }
});

// Methods
const removeFromFavorites = (id) => {
  favoritePosts.value = favoritePosts.value.filter(post => post.id !== id);
};

const searchFavorites = () => {
  // Implement search functionality
  console.log(`Searching for: ${searchQuery.value}`);
  // Could implement filtering based on search query
};
...
<template>
  <div class="min-h-screen bg-black relative overflow-hidden font-sans">
    ...
    <div class="absolute inset-0 overflow-hidden opacity-60 pointer-events-none">
      ...
      <!-- Grid Lines -->
      <div class="absolute inset-0 bg-gradient-to-b from-transparent via-purple-800/5 to-transparent bg-grid-slate-800/5 bg-[length:50px_50px] backdrop-blur-3xl"></div>
    ...
    <main class="container mx-auto px-4 py-12 relative z-10">
      ...
      <header class="flex justify-between items-center mb-12">
        <div class="relative overflow-hidden h-12">
          <div class="text-white font-bold text-2xl tracking-tighter flex items-center space-x-2">
            <div class="h-10 w-10 relative rounded-xl bg-gradient-to-tr from-purple-700 via-indigo-500 to-purple-400 flex items-center justify-center drop-shadow-glow">
              <div class="absolute inset-0 rounded-xl bg-gradient-to-tr from-purple-700 via-indigo-500 to-purple-400 opacity-75 animate-pulse"></div>
              <span class="text-white text-lg font-bold relative z-10">N</span>
            </div>
            <span class="bg-clip-text text-transparent bg-gradient-to-r from-white via-purple-300 to-indigo-300 text-2xl">NEXUS</span>
          </div>
        </div>
        <div class="flex space-x-3">
          <button @click="toggleTheme" class="btn btn-circle bg-gray-900 border-none text-white hover:bg-gray-800">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h12a2 2 0 012 2v12a4 4 0 01-4 4H7z" />
            </svg>
          </button>
      ...
      <!-- Filter Bar -->
      <div class="bg-gray-900 bg-opacity-80 rounded-2xl p-6 mb-10 backdrop-blur-md border border-gray-800 shadow-glow relative">
        <div class="absolute -top-3 -right-3">
          <div class="h-6 w-6 rounded-full bg-gradient-to-tr from-purple-500 to-indigo-400 animate-ping opacity-75"></div>
        </div>
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center space-x-4">
            <div class="badge badge-lg bg-gradient-to-r from-purple-500 to-indigo-500 text-white border-none font-medium">
              {{ favoriteCount }} Favorites
            </div>
            <div class="dropdown">
              <div tabindex="0" class="btn btn-sm btn-ghost text-white glass flex items-center">
                Sort By <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
              </div>
              <ul tabindex="0" class="dropdown-content menu p-3 shadow bg-gray-900 backdrop-blur-md rounded-box w-52 border border-gray-800">
          ...
          <div class="w-full sm:w-auto">
            <div class="join relative w-full sm:w-auto">
              ...
              <div class="absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-purple-500 via-indigo-400 to-pink-500 w-full"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mb-12 flex flex-wrap gap-2">
        <button 
          class="px-6 py-3 rounded-xl relative group text-center"
        ...
        >
          ...
          <span v-if="activeCategory === 'all'" class="absolute bottom-0 left-0 w-full h-full rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 blur-md opacity-50"></span>
        </button>
        <button 
          class="px-6 py-3 rounded-xl relative group text-center"
        ...
        >
          ...
          <span v-if="activeCategory === 'articles'" class="absolute bottom-0 left-0 w-full h-full rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 blur-md opacity-50"></span>
        </button>
        <button 
          class="px-6 py-3 rounded-xl relative group text-center"
        ...
        >
          ...
          <span v-if="activeCategory === 'tutorials'" class="absolute bottom-0 left-0 w-full h-full rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 blur-md opacity-50"></span>
        </button>
        <button 
          class="px-6 py-3 rounded-xl relative group text-center"
        ...
        >
          ...
          <span v-if="activeCategory === 'news'" class="absolute bottom-0 left-0 w-full h-full rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 blur-md opacity-50"></span>
        </button>
        <button 
          class="px-6 py-3 rounded-xl relative group text-center"
        ...
        >
          ...
          <span v-if="activeCategory === 'resources'" class="absolute bottom-0 left-0 w-full h-full rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 blur-md opacity-50"></span>
        </button>
      </div>

      <!-- Loading Skeleton -->
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="i in 6" :key="i" class="card bg-gray-900 bg-opacity-70 backdrop-blur-md rounded-2xl overflow-hidden animate-pulse">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          <!-- Card Glow Effect -->
          <div class="absolute -inset-0.5 bg-gradient-to-r from-purple-600/30 to-indigo-600/30 rounded-2xl blur opacity-0 group-hover:opacity-100 transition duration-300"></div>
          ...
          <div class="card relative bg-gray-900 rounded-2xl overflow-hidden border border-gray-800">
            <figure class="relative">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-300" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div class="absolute bottom-3 left-3 flex space-x-2">
                <div class="badge badge-sm bg-purple-600 bg-opacity-80 backdrop-blur-sm text-white border-none">
                  {{ post.category }}
                </div>
                <div class="badge badge-sm bg-gray-800 bg-opacity-80 backdrop-blur-sm text-gray-200 border-none">
            ...
            <div class="card-body p-6">
              ...
              <div class="w-full h-1 bg-gray-800 rounded-full mb-4 overflow-hidden">
                <div class="h-full bg-gradient-to-r from-purple-500 to-indigo-500" :style="`width: ${Math.floor(Math.random() * 100)}%`"></div>
              </div>
              
              <div class="flex items-center justify-between mt-auto">
                <div class="flex items-center space-x-2">
                  <div class="avatar">
                    <div class="w-8 h-8 rounded-full ring ring-purple-500 ring-offset-black ring-offset-2">
              ...
              <div class="card-actions justify-end mt-4 gap-3">
                <button class="btn btn-sm bg-transparent border border-gray-700 text-purple-300 hover:bg-purple-900 hover:text-purple-100">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                  </svg>
                  Share
                </button>
                <button class="btn btn-sm bg-gradient-to-r from-purple-600 to-indigo-600 border-none text-white hover:from-purple-700 hover:to-indigo-700">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <!-- Empty State -->
      <div v-if="filteredPosts.length === 0 && !loading" class="flex flex-col items-center justify-center py-20 px-4">
        ...
        <p class="text-purple-200 text-center max-w-md mb-8">Looks like you haven't added any favorites in this category yet. Explore more content and mark your favorites!</p>
        <button class="btn btn-lg bg-gradient-to-r from-purple-600 to-indigo-600 border-none text-white hover:opacity-90">
