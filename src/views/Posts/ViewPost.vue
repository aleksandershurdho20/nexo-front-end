...
const submitComment = async () => {
    ...
    try {
        ...
        // Initialize replies array for the new comment
        ...
        // Initialize reply content storage for the new comment
        data.replyContent[newComment.id] = "";
...
const toggleReplyInput = async (commentId, postId) => {
    showReplyInput[commentId] = !showReplyInput[commentId];
    
    replyTarget[commentId] = commentId;

    if (!replies[commentId]) {
        const { data } = await api.get(`/replies/${commentId}`);
        
        replies[commentId] = data;

        if (showReplyInput[commentId]) {
            Object.keys(showReplyInput).forEach(id => {
...
const submitReply = async (commentId) => {
    ...
    const params = {
        ...
        content: data.replyContent[commentId],
        // is_reply: true
    };
    
    try {
        const response = await api.post("/replies", params);
...
<template>
        ...
        <div v-else-if="post" class="space-y-8">
                ...
                <div class="flex items-center justify-between mt-8 pt-4 border-t border-gray-700">
                    <div class="flex space-x-6">
                        <span class="flex items-center text-gray-400 text-sm">
                            ...
                            {{ post.post_insight.views || '1.2k' }} views
                        </span>
                        <span class="flex items-center text-gray-400 text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-gray-800 bg-opacity-50 rounded-xl p-6 border border-gray-700 backdrop-blur-sm">
                ...
                <div v-else id="comments-container" class="space-y-6 max-h-[600px] overflow-y-auto pr-2">
                    <div v-for="comment in post.comments" :key="comment.id" 
                        ...
                        <div class="flex gap-4 mt-3">
                            ...
                            <button @click="toggleReplyInput(comment.id,comment.id)" :class="[
                                'text-xs transition-colors flex items-center',
                                showReplyInput[comment.id] ? 'text-purple-400' : 'text-gray-400 hover:text-purple-400'
                            ]">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                        ...
                        <!-- Reply input -->
                        <div v-if="showReplyInput[comment.id]" class="mt-3 pl-4 border-l-2 border-purple-500 transition-all duration-300">
                            <div class="flex">
                                <input 
                                    v-model="data.replyContent[comment.id]" 
                                    type="text" 
                                    placeholder="Write a reply..."
                                    class="flex-grow bg-gray-600 bg-opacity-50 text-white text-sm rounded-l-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500" 
                                />
                                <button 
                                    @click="submitReply(comment.id)"
                                    class="bg-purple-600 hover:bg-purple-700 text-white text-sm px-4 py-2 rounded-r-lg transition-colors"
                                >
                                    Reply
                                </button>
                            </div>
                         ...
                         {{ console.log(replies) }}
                        <div v-if="replies[comment.id] && replies[comment.id].length > 0" :id="`replies-${comment.id}`" class="mt-4 space-y-3 pl-6 border-l border-gray-600">
                            <div v-for="reply in replies[comment.id]" :key="reply.id" 
                                class="bg-gray-700 bg-opacity-30 rounded-lg p-3 border border-gray-600 hover:border-purple-400 transition-all duration-300">
                                <!-- Reply content -->
                                <div class="flex items-center mb-2">
                                    <div class="h-6 w-6 rounded-full bg-gradient-to-r from-purple-500 to-blue-400 mr-2"></div>
                                    <div>
                                        <p class="text-white text-xs font-medium">{{ reply.user_id || 'Anonymous User' }}</p>
                                        <p class="text-gray-400 text-xs">{{ formatDate(reply.created_at) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex gap-3 mt-2">
                                    <button @click="toggleCommentLike(reply)" :class="[
                                        'text-xs transition-colors flex items-center',
                ...
                <div v-if="post.comments && post.comments.length > 0" class="mt-6 text-center">
